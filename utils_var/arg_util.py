...
class Args(Tap):
    data_path: str = 'torchvision_ImageFolder'
    exp_name: str = 'text'
...
def init_dist_and_get_args(default_args=None) -> Args:
    ...
    # Initialize arguments as a dictionary
    args_defaults = args_defaults = {
        key: getattr(Args, key, None) for key in Args.__annotations__.keys()
    }

    # Create OmegaConf object for Args
    args_conf = OmegaConf.create(args_defaults)
    
    # Merge default_args into args
    args = OmegaConf.merge(default_args, args_conf)

    # Handle local debug mode
    if args.local_debug:
        args.pn = '1_2_3'
        args.seed = 1
        args.aln = 1e-2
    ...
    # Update batch size and learning rate arguments
    ...
    # Update progressive training arguments
    if args.pgwp == 0:
