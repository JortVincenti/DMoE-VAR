...
class AmpOptimizer(Optimizer):
    def __init__(
        ...
        para_groups = optimizer.param_groups
    ...
    def step(self, closure=None):
        # If you have custom logic (AMP scaling, etc.) you do it here.
        # Or if you want to rely on the base_optimizer's step, just do:
        return self.base_optimizer.step(closure=closure)

    def zero_grad(self, set_to_none=True):
        return self.base_optimizer.zero_grad(set_to_none=set_to_none)

    # You might also need to override e.g. add_param_group, load_state_dict, state_dict, etc.
    # so that the LR scheduler and PyTorch can handle everything seamlessly.
    ...
    @param_groups.setter
    def param_groups(self, new_value):
