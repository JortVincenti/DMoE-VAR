...
def count_scaled_dot_product_attention_ops(inputs, _outputs):
    """
    Rough FLOP estimate for torch.scaled_dot_product_attention.
    Accepts Q (B, Hq, Tq, D), K (B, Hk, Tk, D), V (B, Hv, Tv, D)
    """
    q_s, q_h, Tq, D = get_shape(inputs[0])
    k_s, k_h, Tk, _ = get_shape(inputs[1])
    v_s, v_h, Tv, _ = get_shape(inputs[2])

    # (1) QKᵀ
    matmul_qk = q_s * q_h * Tq * Tk * D
    # (2) /√D
    divs      = q_s * q_h * Tq * Tk
    # (3) +mask (if any)
    mask_add  = divs if (get_shape(inputs[3]) is not None
                         or isinstance(inputs[3], numbers.Number)) else 0
    # (4) softmax
    softmax   = 2 * divs
    # (5) AV
    matmul_av = q_s * q_h * Tq * Tv * D    # Tv == Tk in standard SDPA

    return Counter({
        "matmul": matmul_qk + matmul_av,
        "div":    divs,
        "add":    mask_add,
        "softmax": softmax,
    })
...
def retrieve_final(args, run_name: str, device: Union[torch.device, str] = 'cpu'):
    if 'var' in run_name:
        final_path = Path('DMoE-VAR/shared/results/effbench_runs/TINYIMAGENET_PATH_var_d16_GCJYOT4E_1/final.pth')
    elif 'distill' in run_name:
        final_path = Path('DMoE-VAR/shared/results/effbench_runs/TINYIMAGENET_PATH_mha_rep_distill_MFR6DQD4_1/final.pth')
    elif 'enforce_sparsity' in run_name:
        final_path = Path('DMoE-VAR/shared/results/effbench_runs/TINYIMAGENET_PATH_enforce_sparsity_C5DOGOFE_1/final.pth')
    elif 'dsti_expert_split' in run_name:
        final_path = Path('DMoE-VAR/shared/results/effbench_runs/TINYIMAGENET_PATH_dsti_expert_split_4KAHVWXI_1/final.pth')
    elif 'dsti_router' in run_name:
        final_path = Path('DMoE-VAR/shared/results/effbench_runs/TINYIMAGENET_PATH_dsti_router_XOPFHBXX_1/final.pth')
    else:
...
def load_model(args, exp_name: str, exp_id: str, device: Union[torch.device, str] = 'cpu'):
    ...
    for arg, model_arg in zip(reversed(run_args), reversed(run_model_args)):
        ...
        if arg.base_on is None:
            ...
            if 'relu' in arg.dsti_enforce_mode and False:
