...
class VAR(nn.Module):
    ...
    def set_scale_switch(self, scale_switch=0):
        for b in self.blocks:
            b.scale_switch=scale_switch
        ...
        def _toggle_ffn_pruning(blocks, use_pruned: bool, pct_remove: float = 0.10):
            for b in blocks:
                ...
                if use_pruned and not hasattr(b, "_ffn_pruned"):
                    pruned = copy.deepcopy(full_ffn)
                    pruned.prune_by_least_impact(pct_remove)
    ...
    def autoregressive_infer_cfg(
        ...
        for si, pn in enumerate(self.patch_nums):  # si: i-th segment
            ...
            x = next_token_map
            block_output = []
            layer_sparsity_current_scale = []
            if si < self.blocks[-1].scale_switch:
                for b in self.blocks:
                    x, sparsity = b.dense_blocks(x=x, cond_BD=cond_BD_or_gss, attn_bias=None)
                    layer_sparsity_current_scale.append(sparsity.mean().item())
                    block_output.append(x.detach().cpu().clone())
                    if si == b.scale_switch-1:
                        src_attn, dst_attn = b.dense_blocks.attn, b.attn
                        dst_attn.cached_k = src_attn.cached_k    # share the same tensor
                        dst_attn.cached_v = src_attn.cached_v
            else:
                for b in self.blocks:
                    x, sparsity = b(x=x, cond_BD=cond_BD_or_gss, attn_bias=None)
                    layer_sparsity_current_scale.append(sparsity.mean().item())
                    block_output.append(x.detach().cpu().clone())
            
            layer_sparsity_all_scales.append(layer_sparsity_current_scale)
            final_sparsity_mean.append(sparsity.mean(dim=0).mean().item())
            sparsity_max, _ = sparsity.max(dim=1)
            sparsity_min, _  = sparsity.min(dim=1)

            final_sparsity_std.append((sparsity_max - sparsity_min).mean().item())
            logits_BlV = self.get_logits(x, cond_BD, si)
            ...
            final_img = self.vae_proxy[0].fhat_to_img(f_hat.clone())
        ...
        if layer_sparsity_all_scales:
            layer_sparsity_tensor = torch.tensor(layer_sparsity_all_scales, device=self.lvl_1L.device)
            avg_layer_sparsity = torch.mean(layer_sparsity_tensor, dim=0)
        else:
            avg_layer_sparsity = torch.zeros(len(self.blocks), device=self.lvl_1L.device)

        return self.vae_proxy[0].fhat_to_img(f_hat).add_(1).mul_(0.5), final_sparsity_mean, final_sparsity_std, avg_layer_sparsity
